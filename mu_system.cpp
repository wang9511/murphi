...
statesCurrentLevel(0), statesNextLevel(0), currentLevel(0), pno(1.0)
{
  ...
  if (!args->print_64bit_info.value) {
    ...
    if (createqueue) {
      queue =
	  new
	  state_queue((unsigned long) (gPercentActiveStates * NumStates));
    } else {
      queue =
	  new
	  state_stack((unsigned long) (gPercentActiveStates * NumStates));
    }
#if __WORDSIZE == 64
  }
#endif
  ...
  if (!args->find_errors.value){
    ...
    for (what_invariant = 0; what_invariant < numinvariants;
        ...
        /* Uh oh, invariant blown. */
      {
        ...
        //<< invariants[what_invariant].name << "\n";
      }
      // "Invariant \"%s\" failed.",
      // else *invariants[what_invariant].failed = FALSE;
    }
  }
  return TRUE;
}
